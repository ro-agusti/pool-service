...
export const load: PageServerLoad = async ({ locals, url }) => {
	...
	const { data: weekVisits } = await (isAdmin
		? weekQuery
		: weekQuery.eq('technician_id', locals.user!.id));
	...
	const { data: monthVisits } = await (isAdmin
		? monthQuery
		: monthQuery.eq('technician_id', locals.user!.id));
  ...
  `
	...
	const { data: backlog } = await (isAdmin
		? backlogQuery
		: backlogQuery.eq('technician_id', locals.user!.id)); // Weekly coverage (admin only)

	let weeklyCoverage: any[] = [];
	if (isAdmin) {
		const { data: weekVisitsAll } = await admin
			.from('visits')
			.select(
				`
      id, scheduled_date, technician_id, status,
      properties ( id, address, suburb, lat, lng,
        customers ( name )
      )
    `
			)
      .eq('org_id', locals.user!.org_id) 
		...
		weeklyCoverage = weekDays.map((date) => {
			const dow = dowOf(date);
			const dayVisits = (weekVisitsAll ?? []).filter((v: any) => v.scheduled_date === date);

			const techGroups = (allTechs ?? []).map((tech: any) => {
				// working_days usa 1=Mon...7=Sun, dowOf usa 0=Mon...6=Sun
				const worksThisDay = (tech.working_days ?? [1, 2, 3, 4, 5]).includes(dow + 1);
				const visits = dayVisits.filter((v: any) => v.technician_id === tech.id);
				const suburbs = [...new Set(visits.map((v: any) => v.properties?.suburb).filter(Boolean))];
				return {
					techId: tech.id,
					techName: tech.name,
					worksThisDay,
			...
			const unassigned = dayVisits.filter((v: any) => !v.technician_id).length;

			return { date, dow, techGroups, unassigned };
		});
	}

  // Technicians for reschedule selector (admin only)
let technicians: any[] = []
if (isAdmin) {
...
export const actions: Actions = {
  ...
  rescheduleVisit: async ({ request, locals }) => {
  const form = await request.formData()
  const visitId      = form.get('visitId') as string
  const newDate      = form.get('newDate') as string
  const newTechId    = form.get('newTechId') as string
  const oldDate      = form.get('oldDate') as string
  const oldTechId    = form.get('oldTechId') as string
  ...
  await admin.from('visit_logs').insert({
    ...
    old_status: 'pending',
    new_status: 'pending',
    reason: `Rescheduled${newTechId !== oldTechId ? ' + technician changed' : ''}`
  })
},
