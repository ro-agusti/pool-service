...
export const load: PageServerLoad = async ({ locals }) => {
	...
	const { data: team } = await admin
		.from('users')
		.select('id, name, email, role, phone, address, working_days, active, away_from, away_to')
		.eq('org_id', locals.user!.org_id)
		.order('role')
		.order('name');

	// Compute status for each member
	const teamWithStatus = (team ?? []).map((m: any) => {
		let status: 'active' | 'away' | 'deactivated' = 'active';
		if (m.active === false) {
			status = 'deactivated';
		} else if (m.away_from && m.away_to && today <= m.away_to) {
			status = 'away';
		}
		return { ...m, status };
	});

	return { team: teamWithStatus };
};
...
export const actions: Actions = {
	...
	deleteTechnician: async ({ request, locals }) => {
		...
		if (userId === locals.user!.id) return fail(400, { error: "You can't delete yourself" });
