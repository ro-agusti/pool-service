...
<div class="max-w-2xl">
        ...
        <div class="flex items-center gap-2">
          ...
          {#if confirmDeleteCustomer}
            <form method="POST" action="?/deleteCustomer" use:enhance class="flex items-center gap-1">
              ...
              <button type="submit" class="px-3 py-1.5 bg-danger text-white text-sm rounded-lg hover:opacity-90">Yes</button>
              <button type="button" onclick={() => confirmDeleteCustomer = false}
                class="px-3 py-1.5 border border-border text-sm rounded-lg hover:bg-surface">No</button>
            </form>
          {:else}
            <button onclick={() => confirmDeleteCustomer = true}
  ...
  <div class="mt-8 flex items-center justify-between mb-3">
    ...
    {#if isAdmin}
      <a href="/customers/{customer.id}/properties/new"
              ...
              <div class="flex items-center gap-1">
                ...
                {#if confirmDeletePropertyId === property.id}
                  <form method="POST" action="?/deleteProperty" use:enhance={() => {
                    ...
                    <button type="submit" class="text-xs px-2 py-1.5 bg-danger text-white rounded-lg hover:opacity-90">Yes</button>
                    <button type="button" onclick={() => confirmDeletePropertyId = null}
                      class="text-xs px-2 py-1.5 border border-border rounded-lg hover:bg-surface">No</button>
                  </form>
                {:else}
                  <button onclick={() => confirmDeletePropertyId = property.id}
